...
```
Installs agents, `instructions.md`, `.mcp.json` (tool paths made absolute), templates, and a per-agent `logs/` dir in one shot — cross-platform, re-runnable (`--force` to overwrite). See root `README.md` for details.
...
`project-context.md` (what/why + design + decisions) · `coding-standards.md` · `task-board.md` · `design.md` (UI projects) · `logs/<agent>.md` (one file per agent — that agent appends only, so parallel agents never collide and no lock is needed). architect owns the technical record, PM owns the project record. User-facing docs split three ways: architect (overview/setup) · senior-dev (API/usage reference) · tester (verified how-to). "Analyze the code" = Grep/Glob/Read. See `instructions.md`.
